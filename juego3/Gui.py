...
#    pygame.mouse.set_visible(0)
    #Si se quiere un size determinado de pantalla
    #self.screen=pygame.display.set_mode(resolution,pygame.RESIZABLE)
    #Si se quiere pantalla completa
    self.screen=pygame.display.set_mode(resolution,pygame.FULLSCREEN)
#    self.screen=pygame.display.set_mode(resolution)
  
    self.snd_startup=Tools.load_sound(appdir,"startup.wav")
    self.snd_right=Tools.load_sound(appdir,"ok.ogg")
    self.snd_wrong=Tools.load_sound(appdir,"drums.aiff")
...
# WAIT FOR ANSWERS 
  def wait_for_answers(self):
    ...
    while 1:
      ...
      if buttons[1] or teclado[K_ESCAPE]:
         sys.exit("Bye!")
